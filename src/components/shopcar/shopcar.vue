		...
		<div class = "content" @click = "toggleList()">
			<div class = "content-left">
				<div class = "logo-wrapper">
					...
					<div class = "num" v-show="totalCount > 1">{{totalCount}}</div>
				</div>
	...
	export default {
		...
		props: {
			selectFoods: {
				type: Array,
				default: function () {
					// 在props中，数组类型对象类型的数据，默认值需要通过方法来设定
					return [];
				}
			},
			deliverPrice: {
		...
		computed: {
			...
			listShow() {
				...
				if (show) {
					this.$nextTick(() => {
						if (!this.scroll) {
							this.scroll = new BuildScroll(this.$els.listContent, {
								click: true
							});
						} else {
							this.scroll.refresh();
		...
		transitions: {
			drop: {
				beforeEnter(el) {
					...
					console.log(1, this.balls.length);
					while (count--) {
